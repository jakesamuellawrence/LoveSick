    ...
    public void Draw(CustomGameChip gameChip) {
        ...
        if (gameChip.dayPhase == 0) {
            ...
            if (gameChip.currentDate == 1) {
                ...
                string message = "Today, you are the meal. You want to leave, but he convinces you to stay. Isn't it kind of nice to feel wanted for once? It wouldn't be fair to just let him starve after all he's done for you, right?";
                DrawLines(gameChip, message);
            }
        }
        else if (gameChip.dayPhase == 2) {
            Console.WriteLine("day phase is 2");
            titleText += "Dessert";
            Console.WriteLine("Drawing Title");
            gameChip.DrawText(titleText, 16, 8, DrawMode.Sprite, "large", 15, -2);
            Console.WriteLine("Done drawing title");
            if (gameChip.currentDate == 1) {
                ...
                Console.WriteLine("Drawing message");
                DrawLines(gameChip, message);
                Console.WriteLine("Done drawing message");
            }
            else if (gameChip.currentDate == 2) {
